	...
	return (
		<div className="rounded-lg border bg-white h-full">
			...
			<div className="h-[85%] overflow-y-auto px-3 pb-4 ">
				{
					problem?.test_cases.map((item, i) => {
						return (
							<div className="flex flex-col border-b border-b-gray-300" key={i}>
								<h4 className="mt-2 font-bold">{t('input')}:</h4>
								<p className="my-1 whitespace-pre">{item.input}</p>
								<h4 className="mt-2 font-bold">{t('output')}:</h4>
								<p className="my-1 whitespace-pre">{item.output}</p>
							</div>
						)
					})
				}
